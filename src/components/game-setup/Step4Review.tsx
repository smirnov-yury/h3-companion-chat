        ...
        onClick={async () => {
          ...
          try {
            ...
            const [scenRes, blocksRes, mvRes, teRes, heroesRes, unitsRes, buildingsRes, aiSetupRes, storySectionsRes, ipHash] = await Promise.all([
              ...
              heroIds.length
                ? supabase.from("heroes").select("id, name_en, name_ru, image, attack, defense, power, knowledge, class_en, class_ru, updated_at").in("id", heroIds)
                : Promise.resolve({ data: [], error: null } as const),
              towns.length
                ? supabase.from("unit_stats").select("id, name_en, name_ru, town, tier, number, cost, image, updated_at").in("town", towns)
                : Promise.resolve({ data: [], error: null } as const),
              townIdsLower.length

...
export default function GuidePanelContentEditor({ panel, sectionSlug, panelSort, onSaved }: Props) {
  ...
  const setLocPath = (path: (string | number)[], val: string) => update((c) => {
    const o = walk(c, path); const k = path[path.length - 1];
    o[k] = o[k] ?? {}; o[k][lang] = val;
    if (!o[k].en && !o[k].ru) delete o[k];
  });
  const setStrPath = (path: (string | number)[], val: string) => update((c) => {
    const o = walk(c, path); const k = path[path.length - 1];
    if (val) o[k] = val; else delete o[k];
  });
  const setNumPath = (path: (string | number)[], val: string) => update((c) => {
    const o = walk(c, path); const k = path[path.length - 1];
    if (val === "") delete o[k]; else o[k] = Number(val);
  });
  const setBoolPath = (path: (string | number)[], val: boolean) => update((c) => {
    const o = walk(c, path); const k = path[path.length - 1];
    if (val) o[k] = true; else delete o[k];
  });

  const loc = (obj: any, k: string) => obj?.[k]?.[lang] ?? "";

  const moveItem = (key: string, i: number, dir: number) => update((c) => {
    const arr = Array.isArray(c[key]) ? c[key] : []; const j = i + dir;
    if (j < 0 || j >= arr.length) return;
    [arr[i], arr[j]] = [arr[j], arr[i]]; c[key] = arr;
  });
  ...
  const imgSlot = (slotKey: string, current: string | null, imgPath: (string | number)[]) => {
    ...
    return (
      <div className="w-32 shrink-0">
        <ImageUploader
          table="guide_panels" recordId={panel.id} folder="guide" imageField="image_path"
          currentImage={base} skipDbUpdate hasImageStatus={false} defaultCropPreset="free"
          filename={(filenames[fk] && filenames[fk].trim()) || base || slotName(slotKey)}
          onUploaded={(fn) => { if (fn) setImageAt(imgPath, fn); }}
          onDeleted={() => setImageAt(imgPath, null)}
        />
        <input value={filenames[fk] ?? base ?? ""} onChange={(e) => setFilenames((p) => ({ ...p, [fk]: e.target.value }))}
          placeholder={slotName(slotKey)} className="w-32 mt-1 bg-transparent text-[10px] font-mono text-muted-foreground outline-none border-b border-border focus:border-primary px-1" />
      </div>
    );
  };
  ...
  const advGlyph = (obj: any, path: (string | number)[]) => adv ? (
    <div><label className="block text-[10px] text-muted-foreground mb-0.5">Glyph</label>
      <input value={obj.glyph ?? ""} onChange={(e) => setStrPath(path, e.target.value)} className={`${FIELD} w-40`} /></div>
  ) : null;
  ...
  return (
    <div className="space-y-4 border border-border rounded-md p-3 bg-muted/20">
      <div className="flex items-center justify-between">
        <div className="inline-flex border border-border rounded-md overflow-hidden">
          {(["en", "ru"] as const).map((l) => (
            <button key={l} type="button" onClick={() => setLang(l)} className={`text-xs px-3 py-1 ${lang === l ? "bg-primary text-primary-foreground" : "bg-transparent text-muted-foreground"}`}>{l.toUpperCase()}</button>
          ))}
        </div>
      ...
      {kind === "anatomy" && (
        <div className="space-y-2">
          ...
          <div>
            ...
            <textarea rows={3} className={`${FIELD} resize-y`}
              value={Array.isArray(content.intro?.[lang]) ? content.intro[lang].join("\n") : ""}
          ...
          <Section title={`Points (${points.length})`} onAdd={() => addItem("points", { label: {}, detail: {} })} addLabel="Add point">
            {points.map((p, i) => (
              <div key={i} className="border border-border rounded-md p-2.5 space-y-2 bg-background">
                ...
                <div className="flex gap-3">
                  <div className="flex-1 space-y-2">
                    {Lfield("Label", p, "label", ["points", i, "label"])}
                    {Lfield("Detail title", p.detail ?? {}, "title", ["points", i, "detail", "title"])}
                    {Lfield("Detail text", p.detail ?? {}, "text", ["points", i, "detail", "text"], true)}
                  </div>
                  {imgSlot(`point${i + 1}`, p?.detail?.image ?? null, ["points", i, "detail", "image"])}
                </div>
              </div>
            ))}
          </Section>
          <Section title={`Items (${items.length})`} onAdd={() => addItem("items", { label: {} })} addLabel="Add item">
            {items.map((it, i) => (
              <div key={i} className="border border-border rounded-md p-2.5 space-y-2 bg-background">
                ...
                <div className="flex gap-3">
                  <div className="flex-1 space-y-2">
                    {Lfield("Label", it, "label", ["items", i, "label"])}
                    {Lfield("Text", it, "text", ["items", i, "text"], true)}
                    {advGlyph(it, ["items", i, "glyph"])}
                    {adv && <div><label className="block text-[10px] text-muted-foreground mb-0.5">Link route</label><input value={it.route ?? ""} onChange={(e) => update((c) => { const v = e.target.value; if (v) { c.items[i].route = v; c.items[i].mode = "open"; } else { delete c.items[i].route; delete c.items[i].mode; } })} className={FIELD} /></div>}
                    {adv && Lfield("Link label", it, "target", ["items", i, "target"])}
                  </div>
                  {imgSlot(`item${i + 1}`, it?.image ?? null, ["items", i, "image"])}
                </div>
              </div>
            ))}
          </Section>
      ...
      {kind === "anatomy" && (
        <>
          <Section title={`Callouts (${callouts.length})`} onAdd={() => addItem("callouts", { pin: (callouts.reduce((m, c) => Math.max(m, typeof c.pin === "number" ? c.pin : 0), 0) + 1), label: {} })} addLabel="Add callout">
            {callouts.map((c0, i) => (
              <div key={i} className="border border-border rounded-md p-2.5 space-y-2 bg-background">
          ...
          <Section title={`Abilities (${abilities.length})`} onAdd={() => addItem("abilities", { label: {} })} addLabel="Add ability">
            {abilities.map((a, i) => (
              <div key={i} className="border border-border rounded-md p-2.5 space-y-2 bg-background">
                ...
                <div className="flex gap-3">
                  <div className="flex-1 space-y-2">{Lfield("Label", a, "label", ["abilities", i, "label"])}{Lfield("Text", a, "text", ["abilities", i, "text"], true)}{advGlyph(a, ["abilities", i, "glyph"])}</div>
                  {imgSlot(`ability${i + 1}`, a?.image ?? null, ["abilities", i, "image"])}
                </div>
              </div>
            ))}
          </Section>
          ...
          <Section title={`Types (${types.length})`} onAdd={() => addItem("types", { label: {} })} addLabel="Add type">
            {types.map((t, i) => (
              <div key={i} className="border border-border rounded-md p-2.5 space-y-2 bg-background">
                ...
                <div className="flex gap-3">
                  <div className="flex-1 space-y-2">{Lfield("Label", t, "label", ["types", i, "label"])}{Lfield("Short", t, "short", ["types", i, "short"])}{Lfield("Detail text", t, "text", ["types", i, "text"], true)}{advGlyph(t, ["types", i, "glyph"])}</div>
                  {imgSlot(`type${i + 1}`, t?.image ?? null, ["types", i, "image"])}
                </div>
              </div>
            ))}
          </Section>
          <Section title={`Tiers (${tiers.length})`} onAdd={() => addItem("tiers", { label: {} })} addLabel="Add tier">
            {tiers.map((t, i) => (
              <div key={i} className="border border-border rounded-md p-2.5 space-y-2 bg-background">
                ...
                <div className="flex gap-3">
                  <div className="flex-1 space-y-2">{Lfield("Label", t, "label", ["tiers", i, "label"])}{Lfield("Text", t, "text", ["tiers", i, "text"], true)}{advGlyph(t, ["tiers", i, "glyph"])}</div>
                  {imgSlot(`tier${i + 1}`, t?.image ?? null, ["tiers", i, "image"])}
                </div>
              </div>
            ))}
          </Section>

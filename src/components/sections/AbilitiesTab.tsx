    ...
    <>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && closeCard()}>
        <CardDialogContent>
          {selected && (
            <>
              <div className="relative w-[85%] mx-auto pt-4 mb-0 shrink-0 h-[280px] bg-muted/50 rounded-lg">
                <img src={selected.image_regular ? `${STORAGE}/abilities/${selected.image_regular}` : DECK_PLACEHOLDER} alt={selected.name_en} className="w-full h-[280px] object-contain rounded-lg shadow-lg" onError={(e) => { e.currentTarget.src = DECK_PLACEHOLDER; e.currentTarget.onerror = null; }} />
              </div>

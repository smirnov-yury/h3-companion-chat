...
export default function AstrologersTab({ searchQuery = "", initialCardId, onCardOpen, onCardClose }: Props) {
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("astrologers_proclaim").select("description_en, description_ru, effect_en, effect_ru, id, image, image_status, name_en, name_ru, notes_en, notes_ru, sort_order, updated_at").order("sort_order");
            ...
            <>
              {selected.image && (
                <div className="w-full shrink-0 flex justify-center bg-muted">
                  <ImageWithSpinner src={componentImageUrl("astrologers_proclaim", selected.image, selected.updated_at)} alt={selected.name_en} className="w-full max-h-[280px] object-contain" />
                </div>
              )}
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                ...
                <SeeAlso entityType="astrologer" entityId={selected.id} lang={lang} />
              </div>
            </>
          )}
        </CardDialogContent>

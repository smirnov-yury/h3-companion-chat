...
export default function MapEventsTab({ searchQuery = "", initialCardId, onCardOpen, onCardClose }: Props) {
  ...
  const name = (i: MapEvent) => lang === "RU" ? (i.name_ru || i.name_en) : i.name_en;

  const q = searchQuery.toLowerCase();
  const filtered = searchQuery
    ? items.filter(i => [i.name_en, i.name_ru, i.description_en, i.description_ru]
        .some(f => f && f.toLowerCase().includes(q)))
    : items;

  const selected = selectedIndex !== null ? filtered[selectedIndex] ?? null : null;
  const openCard = (i: MapEvent) => {
      ...
      <div className="p-3 overflow-y-auto h-full">
        {!loaded ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {filtered.map((item) => {
              const imgSrc = item.image ? componentImageUrl("map_events", item.image, item.updated_at) : null;
              return (
            ...
            <>
              {selected.image && (
                <div className="w-full shrink-0 flex justify-center bg-muted">
                  <ImageWithSpinner src={componentImageUrl("map_events", selected.image as string, selected.updated_at)} alt={selected.name_en} className="w-full aspect-[5/7] object-contain rounded-lg shadow-lg" />
                </div>
              )}
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                ...
                <SeeAlso entityType="map_event" entityId={selected.id} lang={lang} />
              </div>
            </>
          )}
        </CardDialogContent>

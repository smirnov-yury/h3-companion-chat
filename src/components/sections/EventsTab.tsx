...
export default function EventsTab({ searchQuery = "", initialCardId, onCardOpen, onCardClose }: Props) {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ["events"],
    queryFn: async () => {
      const { data, error } = await supabase.from("events").select("effect_en, effect_ru, id, image, image_status, name_en, name_ru, notes_en, notes_ru, sort_order, updated_at").order("sort_order");
      ...
      <div className="p-3 overflow-y-auto h-full">
        {!loaded ? (
          ...
          <CardGrid layout={layout}>
            {filtered.map((item) => {
              const imgSrc = item.image ? componentImageUrl("events", item.image, item.updated_at) : null;
              return (
